...
const getCompositionRecursion = (
  ...
  if (n <= 7 && hai[n] > 0 && hai[n + 1] > 0 && hai[n + 2] > 0) {
    ...
    const cr = getCompositionRecursion(structuredClone(hai), n, structuredClone(ret_a_shuntsu), structuredClone(ret_b_shuntsu)); //抜き取ったら同じ位置でもう一度試行
    const r: number[][] = cr[0];
    ret_a_shuntsu = cr[1];
    ret_b_shuntsu = cr[2];
    hai[n]++;
  ...
  if (hai[n] >= 3) {
    ...
    const cr = getCompositionRecursion(structuredClone(hai), n, structuredClone(ret_a_kotsu), structuredClone(ret_b_kotsu)); //抜き取ったら同じ位置でもう一度試行
    const r: number[][] = cr[0];
    ret_a_kotsu = cr[1];
    ret_b_kotsu = cr[2];
    hai[n] += 3;
...
const getShantenTanyao = (tehai: string): number => {
  ...
  const new_tehai =
    hai_normal.filter((h) => !yaochu.includes(h)).join('') + (hai_furo.length > 0)
...
const getShantenHonitsuByColor = (tehai: string, color: string): number => {
  ...
  const new_tehai =
    hai_normal.filter((h) => [color, 'z'].includes(h.slice(1, 2))).join('') + (hai_furo.length > 0)
      ? hai_furo.map((h) => `<${h}>`).join('')
      : '' + (hai_ankan.length > 0)
        ? hai_ankan.map((h) => `(${h})`).join('')
        : '';
