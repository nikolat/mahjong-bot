...
export const getScore = (
	...
	for (const comp of composition) {
		...
		if (yakuman_sub_max < yakuman_sub
	...
	let count_fu = 0;
	if (ret_han.has('平和') && ret_han.has('門前清自摸和'))
		count_fu = 20;
	else if (ret_han.has('七対子'))
		count_fu = 25;
	else if (!ret_yakuman.has('国士無双'))
		count_fu = fu_sub_max;
...
const getYakuInfoFromComposition = (
	...
	{
		if (is_menzen)
			ret_han.set('混全帯ヤオ九', 2);
		else
			ret_han.set('混全帯ヤオ九', 1);
	}
	if (isJunchan(mentsu, atama_hai))
	{
		if (is_menzen)
			ret_han.set('純全帯ヤオ九', 3);
		else
			ret_han.set('純全帯ヤオ九', 2);
	}
	if (count_peko == 2) {
		ret_han.set('二盃口', 3);
	}
	let fu: number;
	if (ret_han.has('平和') && is_tsumo)
		fu = 20;
	else if (ret_han.has('平和') && !is_tsumo)
		fu = 30;
	else
...
const getFu = (
	...
	if (atama_hai === jifu_hai)
		fu += 2;
	if (['5z', '6z', '7z'].includes(atama_hai))
		fu += 2;
	//(5)待ち
	//待ちの形を再現する
	...
	for (const mt of machi_tatsu) {
		if ((mt[0] !== 1) && (mt[0] + 1 === mt[1]) && (mt[1] !== 9)) {//両面待ち
			fu_machi = 0;
			if (fu_machi_add < fu_machi)
				fu_machi_add = fu_machi;
		}
		else if (mt[0] === mt[1]) {//シャボ待ち
			if (is_tsumo) {
				fu_machi = 0;
				if (fu_machi_add < fu_machi)
					fu_machi_add = fu_machi;
		...
		else if (mt[0] === 1 && mt[1] === 2 || mt[0] === 8 && mt[1] === 9) {//ペンチャン待ち
			fu_machi = 2;
			if (fu_machi_add < fu_machi)
				fu_machi_add = fu_machi;
		}
		else if (mt[0] + 2 === mt[1]) {//カンチャン待ち
			fu_machi = 2;
			if (fu_machi_add < fu_machi)
				fu_machi_add = fu_machi;
		}
...
const subChanta = (mentsu: string[][], atama_hai: string): number => {
	...
	if (yaochu_string.includes(atama_hai)) {
		if (jihai_string.includes(atama_hai))
			has_jihai = true;
	}
	else {
		return 0;
	...
	for (let i = 0; i < mentsu.length; i++) {
		...
		for (let j = 0; j < mentsu[i].length; j++) {
			if (yaochu_string.includes(mentsu[i][j])) {
				if (jihai_string.includes(mentsu[i][j]))
					has_jihai = true;
				has_yaochu = true;
				break;
			}
...
const isChuren = (hai_plain: string[]): boolean => {
	...
	if (churen_array.includes(hai_str))
		return true;
	return false;
};
...
const isIkkitsuukan = (mentsu: string[][]): boolean => {
	...
	if (shuntsu_string.length < 3)
		return false;
	if (['1m', '4m', '7m'].every(h => shuntsu_string.includes(h)))
		return true;
	if (['1p', '4p', '7p'].every(h => shuntsu_string.includes(h)))
		return true;
	if (['1s', '4s', '7s'].every(h => shuntsu_string.includes(h)))
		return true;
	return false;
};
...
const isPinfu = (atama_hai: string, mentsu: string[][], agari_hai: string, bafu_hai: string, jifu_hai: string): boolean => {
	...
	if (atama_hai === '')
		return false;
	//雀頭は役牌以外でないといけない
	if ([bafu_hai, jifu_hai, '5z', '6z', '7z'].includes(atama_hai))
		return false;
	//待ちの形を再現する
	const machi_tatsu: number[][] = [];
	for (const m of mentsu) {
